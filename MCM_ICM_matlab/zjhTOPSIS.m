...
num=[1,2,3,4];% input('请输入要处理的列数,如[1 2 3]');
weight=ones(1,size(num,1));  
Type=[1,1,1,1];% input('请输入对应的类型，1是极小型，2是中间型，3是区间型，如[2 3 1]：');
Judge=0;% input('请问是否需要输入权重？需要则输入1，不需要则输入0：');
if Judge==1
    weight=[];% input('请输入每一列的相应权重,如[0.6 0.9 0.8]：');
end
for i=1:size(num,2)
    if Type(i)==1
...
DPositive=repmat(weight,size(datas2,1),1).*DPositive;
DNegative=((datas2-repmat(min(datas2),size(datas2,1),1)).^2).^0.5;
DNegative=repmat(weight,size(datas2,1),1).*DNegative;
datas3=DNegative./(DPositive+DNegative);%归一化结果
for i=1:size(num,2)
