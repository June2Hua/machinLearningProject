...
for serviceA in range(maxA):
    for serviceB in range(maxB):
        ...
        lambda_A = 47 / (9 * 60 + 56) / (serviceA+1)
        mu_A = (7 + 9) / (
        ...
        lambda_B = (NEW_CUSTOMERS_VIP / lastArrive) * (serviceA + 1) / (serviceB + 1)
...
passengersWaitTime_N = (passengersWaitTime - passengersWaitTime.min()) / (
            passengersWaitTime.max() - passengersWaitTime.min())
...
output = open('nonVIP_passengersWaitTime.xls', 'w', encoding='gbk')
for i in range(len(passengersWaitTime_N)):
    for j in range(len(passengersWaitTime_N[i])):
        output.write(str(passengersWaitTime_N[i][j]))  # write函数不能写int类型的参数，所以使用str()转化
        output.write('\t')  # 相当于Tab一下，换一个单元格
    output.write('\n')  # 写完一行立马换行
output.close()
output = open('nonVIP_utilizationRatio.xls', 'w', encoding='gbk')
